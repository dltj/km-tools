...
class SummarizeAction(ActionBase):
    ...
    def _get_document(self, resource_url: str) -> str:
    ...
    def _get_derived_date(self, resource_url: str, downloaded) -> str:
    ...
    def _get_summarization(self, resource_url: str, downloaded: str) -> str:
        ...
        if len(sentence_list) < 7:
            ...
            sorted_word_frequencies = {
                k: v
                for k, v in sorted(word_frequencies.items(), key=lambda item: item[1])
            }
    ...
    def process(self, session: Session, resource: WebResource) -> None:
        ...
        try:
            downloaded = self._get_document(resource_url)
        except SummarizeError as e:
            raise ActionError(f"Could not process {resource_url}") from e
        derived_date: str = self._get_derived_date(resource_url, downloaded)
        summarization: str = self._get_summarization(resource_url, downloaded)
